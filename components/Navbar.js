...
function Navbar() {
  ...
  const menu = () => {
    return (
      <div className="absolute inset-x-0 top-0 h-44 w-full group-hover:visible invisible">
        <div className="relative h-full w-full">
          <div className="absolute z-20 border border-black min-h-[490px] top-[49px] translate-x-1/2 right-[140%] w-[275px] bg-white px-[38px] py-[30px]">
  ...
  return (
    <nav className="flex shrink-0 items-center h-[50px] border-b border-black justify-between fixed top-0 inset-x-0 bg-white z-20 text-xl">
      <ul className="flex items-center h-full space-x-7 uppercase text-sm">
        ...
        <li className="cursor-pointer select-none flex items-center h-full relative group">
          <Link href="/catalogue" className="block z-20">
            <span>КАТАЛОГ</span>
          </Link>
          {menu()}
        </li>
      ...
      <div className="-translate-x-[40%]">
        <Link href="/" className="h-full flex items-center pl-10 pr-4">
          <svg
      ...
      <ul className="flex items-center space-x-5 uppercase text-sm mr-10">
        <li>войти</li>
        <li>корзина</li>
        <li>ru</li>
      </ul>
